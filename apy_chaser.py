...
def get_candidate_pools(pools):
    candidate_pools = []
    balance_threshold = 1e8
    balance_ratio_threshold = 0.1
    ...
    for pool in pools['data']['poolData']:
        ...
        if total_balance > balance_threshold and min(coin_ratios)>balance_ratio_threshold:
...
def find_max_apy(pools_js, candidate_pools):
    max_apy = -100
    best_pool = ''
    best_pool_address = ''

    for pool in pools_js:
        address = pool['addresses']['swap']
        name = pool[id]
        apy_ = float(apys['data'][name]['baseApy'])
        for pool_ in candidate_pools:
            #print(address, pool_['address'])
            try:
                if address == pool_['address']:
                    print( name, apy_)

                    if apy_ > max_apy :
                        max_apy = apy_
                        best_pool = name
                        best_pool_address = address
            except:
                pass
...
while tracker < 1:
  ...
  pageMetaData: {
    title: 'Compounded',
    description: 'A curve_fi portal for swapping cDAI/cUSDC',
  },
  lpTokenInfo: {
    name: 'cCurve',
    symbol: 'cCrv',
  ...
  pageMetaData: {
    title: 'Tethered',
    description: 'A curve_fi Tethered portal for swapping cDAI/cUSDC/USDT',
  },
  lpTokenInfo: {
    name: 'tCurve',
    symbol: 'tCrv',
  ...
  pageMetaData: {
    title: 'Yield',
    description: 'A curve_fi yTokens portal for swapping DAI/USDC/USDT/TUSD',
  },
  lpTokenInfo: {
    name: 'yCurve',
    symbol: 'yCrv',
  ...
  pageMetaData: {
    title: 'bUSD',
    description: 'A curve_fi portal for swapping BUSD and other stablecoins',
  },
  lpTokenInfo: {
    name: 'bCurve',
    symbol: 'bCrv',
  ...
  referenceAsset: REFERENCE_ASSETS_BTC,
  addresses: {
  ...
  referenceAsset: REFERENCE_ASSETS_EUR,
  addresses: {
  ...
  referenceAsset: REFERENCE_ASSETS_ETH,
  addresses: {
  ...
  referenceAsset: REFERENCE_ASSETS_LINK,
  assets: 'LINK+sLINK',
  coins: [
    coins_link,
    coins_slink,
  ...
  gaugeVersion: 4,
  cryptoPool: True,
  referenceAsset: REFERENCE_ASSETS_CRYPTO,
}, {
  dataIndex: 40,
  id: 'tricrypto2',
  name: 'tricrypto2',
  ...
  referenceAsset: REFERENCE_ASSETS_CRYPTO,
            ...
            },
            {
                ...
                id: 'spelleth',
                name: 'spelleth',
                referenceAsset: REFERENCE_ASSETS_SPELL,
                cryptoPool: True,
                isPlainCryptoPool: True,
                lpTokenInfo: {
                    name: 'SpellEthCurve',
            ...
            },
            {
                ...
                id: 'teth',
                name: 'teth',
                referenceAsset: REFERENCE_ASSETS_T,
                cryptoPool: True,
                isPlainCryptoPool: True,
                lpTokenInfo: {
                    name: 'TEthCurve',
        ...
        #print(e, str(e).split("'")[1])
        name = str(e).split("'")[1]
        globals()[name] = name
        time.sleep(0.1)


print('best pool and apy: ',find_max_apy(pools_js, candidate_pools))